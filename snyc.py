...
# Check if the user has joined the specified channel
...
# Access the token, admin ID, and channel ID from the config.json
...
user_approval_expiry = {}  # To store the expiry time for each approved user
...
def set_approval_expiry_date(user_id, duration, time_unit):
    ...
    if time_unit in ["hour", "hours"]:
        ...
        expiry_date = current_time + timedelta(days=30 * duration)  # Approximation for month
    else:
        return False
...
async def run_attack_command_on_codespace(target_ip, target_port, duration):
    command = f"./bgmi {target_ip} {target_port} {duration} 140"
    try:
...
def approve_user(message):
    ...
    if len(command) > 2:
        ...
        if user_to_approve not in allowed_user_ids:
            ...
            if set_approval_expiry_date(user_to_approve, duration, time_unit):
                bot.reply_to(message, f"User {user_to_approve} approved for {duration} {time_unit}.")
            else:
                bot.reply_to(message, "Failed to set approval expiry date.")
        else:
            bot.reply_to(message, "User already approved.")
    else:
        bot.reply_to(message, "Please specify a user ID and the duration (e.g., 1hour, 2days).")
...
def handle_message(message):
    ...
    # Restrict bot usage to group chats
    if not is_group_chat(message):
        bot.reply_to(message, "You can only use this bot in groups.")
        return

    # Check if user has joined the required channel
    if not is_user_in_channel(user_id):
        bot.reply_to(message, "Please join @creativeydv to use this bot.")
        return

    # Check if user approval is valid
    if not check_user_approval(user_id):
        bot.reply_to(message, "Your approval has expired or you are not approved.")
        return

    # Handle other commands here...
    bot.reply_to(message, "Your message has been processed.")  # Placeholder
